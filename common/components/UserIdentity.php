...
//        logs("aaaaaa");
    ...
    public function authenticateMember() {
        $record = BbiiMember::model()->find(array('condition' => 'username="' . $this->username . '"'));  // check user name from database
        if (empty($record)) {
            // jika member kosong ngambil di USER untuk backend
            $record = User::model()->find(array('condition' => 'username="' . $this->username . '"'));  // check user name from database
            if ($record === null) {
                $this->_id = 'user Null';
                $this->errorCode = "Your user name does not found in our database";
...
//            }else{
                if ($record->id == 1) {
                    ...
                    $sRolesName = 'Member (Undefined Roles)';
                }
//            }

                $this->_id = $record['id'];
                $this->setState('code', $record->code);
                $this->setState('member_name', $record->member_name);
                $this->setState('phone', $record->phone);
                $this->setState('email', $record->email);
                $this->setState('city', $record->city_id);
                $this->setState('address', $record->address);
//            $this->setState('roles_id', $record->roles_id);
                $this->setState('roles_name', $sRolesName);
//            $this->setState('saldo', (isset($record->saldo)) ? $record->saldo : 0);
                $this->setState('avatar', landa()->urlImg('avatar/', $record->avatar, $this->_id));
