...
if(empty($model)){
    echo'<article class="clearfix">
        ...
        <center>Maaf pencarian tidak di temukan.</center>
        </div>
        </article>';
}else{
    ...
    <article class="clearfix">
        ...
        <div class="codo_topics_topic_content">
            ...
            <div class="codo_topics_topic_name">
                <?php echo CHtml::encode($data->poster->member_name); ?>
                <span><span id="posted_txt">posted </span><?php echo ' on ' . date('F d, Y H:i:s', strtotime($data->topic->firstPost->create_time)); ?></span>
            </div>
            <div class="codo_topics_topic_title">
                <?php
                if ($data->topic->sticky == 1) {
                    echo'<u><i class="fa fa-thumb-tack"></i> Sticky</u> :';
                }
                ?>
                <a href="<?php echo url('forum/forum/topic', array('id' => $data->topic->id)) ?>"><?php echo $data->subject ?></a>
                <?php // echo CHtml::link(CHtml::encode($data->title), array('forum/foru', 'id' => $data->id), array('')); ?></a>
...
//            $post = BbiiPost::model()->findByAttributes(array('topic_id' => $data->id));
            echo implode(" ", array_slice(explode(" ", $data->content), 0, 225));
            ?>
        ...
        <div class="codo_topics_topic_foot clearfix">
            ...
            <div class="codo_topics_no_replies"><span><?php echo$data->topic->num_replies; ?></span>dibalas</div>
            <div class="codo_topics_no_replies" id="codo_topics_no_views"><span><?php echo $data->topic->num_views; ?></span>dilihat</div>
            <div class="codo_topics_last_post">
                balasan terakhir dari <a href=""><?php echo $data->topic->lastPost->poster->member_name; ?></a>
                &nbsp;·&nbsp; <span class="codo_topics_last_post_time"><?php echo date('F d, Y H:i:s', strtotime($data->topic->lastPost->create_time)) ?></span>
            </div>
        </div>
