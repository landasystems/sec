...
class BbiiForum extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name', 'required'),
			array('type, public, locked, moderated, sort, num_posts, num_topics, last_post_id,membergroup_id, poll, fjb', 'numerical', 'integerOnly'=>true),
			array('name', 'unique'),
			...
			array('locked, membergroup_id, poll, fjb', 'default', 'value' => 0),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, cat_id, name, subtitle, type, sort, num_posts, num_topics, last_post_id, last_post_time, membergroup_id, poll, fjb', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'topic' => array(self::BELONGS_TO, 'BbiiTopic', 'last_post_id'),
		);
	}
