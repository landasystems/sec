...
class MessageController extends BbiiController {
    ...
    public function accessRules() {
        return array(
            array('allow',
                'actions' => array('create', 'delete', 'inbox', 'outbox', 'reply', 'view', 'viewMessage', 'update', 'sendReport','viewReport'),
                'users' => array('@'),
            ),
...
//        $json = array();
        ...
        if (isset($_POST['BbiiMessage'])) {
            ...
            $model->attributes = $_POST['BbiiMessage'];
            $model->subject = Yii::t('BbiiModule.bbii', 'Post reported: ') . BbiiPost::model()->findByPk($id)->subject;
            $model->sendto = 0;
            $model->sendfrom = Yii::app()->user->id;
            $model->outbox = 0;
            $model->type = 2;
            ...
            if ($model->save()) {
               $this->redirect(array('viewReport'));
            }
        }
        $this->render('sendReport', array(
            'model' => $model,
//            'count' => $count,
        ));
    }
