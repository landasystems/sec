...
class ForumController extends BbiiController {
    ...
    public function accessRules() {
        return array(
            array('allow',
                'actions' => array('createTopic', 'fjbUpload', 'quote', 'reply', 'vote', 'displayVote', 'editPoll', 'updatePoll', 'update', 'upvote', 'markAllRead'),
                'users' => array('@'),
            ),
            array('allow',
...
//                            $modelPhoto->product_id = $topic->id;
...
//                    }
                    ...
                    if (!$forum->moderated) {
                        ...
                        Yii::app()->user->setFlash('moderation', Yii::t('BbiiModule.bbii', 'Postingan anda sudah tersimpan di data kami, harap menunggu moderator atau admin meng-aprove postingan anda.'));
                    }
    ...
    public function actionFjbUpload() {
...
//        $gallery_category = GalleryCategory::model()->findByPk($_GET['id']);
        ...
        $folder = param('pathImg') . '/product/'; // folder for uploaded files
