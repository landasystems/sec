...
class MemberController extends BbiiController {
    ...
    public function accessRules() {
        return array(
            array('allow',
                'actions' => array('index', 'mail', 'members', 'view', 'update','myPost'),
                'users' => array('@'),
            ),
            array('allow',
...
//            $model->code = $_POST['BbiiMember']['code'];
            ...
            $model->type_business = $_POST['BbiiMember']['type_business'];
            $model->company_name = $_POST['BbiiMember']['company_name'];
            $model->address = $_POST['BbiiMember']['address'];
            ...
            if ($model->save()) {
                ...
                user()->setFlash('warning', 'Data berhasil di update.');
    ...
    public function actionRegister() {
        ...
        if (isset($_POST['BbiiMember'])) {
            $cek = BbiiMember::model()->find(array('condition' => 'username="' . $_POST['BbiiMember']['username'] . '"'));
            if (empty($cek)) {
                $model->attributes = $_POST['BbiiMember'];
                $model->password = sha1($_POST['BbiiMember']['password']);
                $model->group_id = 1;
                ...
                $model->addError('username', 'Username telah terdaftar');
            }
        }
    ...
    public function actionMyPost($id){
        $id = user()->id;
        $dataProvider = new CActiveDataProvider('BbiiPost', array(
            'criteria' => array(
                'condition' => "user_id = $id",
