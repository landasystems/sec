...
class TopicController extends Controller {
    ...
    public function actionCari() {
        ...
        $model = BbiiPost::model()->findAll(
                'subject LIKE :match OR content LIKE :match', array(':match' => "%$_GET[cari]%")
        );
...
//            'pages' => $pages,
    ...
    public function actionIndex() {
        ...
        if (isset($_GET['BbiiTopic'])) {
            ...
            if (!empty($model->first_post_id))
                $criteria->addCondition('first_post_id = "' . $model->first_post_id . '"');
            ...
            if (!empty($model->num_replies))
                $criteria->addCondition('num_replies = "' . $model->num_replies . '"');


            if (!empty($model->num_views))
                $criteria->addCondition('num_views = "' . $model->num_views . '"');
            ...
            if (!empty($model->sticky))
                $criteria->addCondition('sticky = "' . $model->sticky . '"');


            if (!empty($model->global))
                $criteria->addCondition('global = "' . $model->global . '"');


            if (!empty($model->moved))
                $criteria->addCondition('moved = "' . $model->moved . '"');
