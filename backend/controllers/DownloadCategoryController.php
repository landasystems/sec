    ...
    public function accessRules() {
        return array(
            array('allow', // c
                ...
                'expression' => 'app()->controller->isValidAccess("DownloadCategory","c")'
            ),
            array('allow', // r
                'actions' => array('index','view'),
                'expression' => 'app()->controller->isValidAccess("DownloadCategory","r")'
            ),
            array('allow', // u
                'actions' => array('update'),
                'expression' => 'app()->controller->isValidAccess("DownloadCategory","u")'
            ),
            array('allow', // d
                'actions' => array('delete'),
                'expression' => 'app()->controller->isValidAccess("DownloadCategory","d")'
            )
        );
    }
            ...
            if ($_POST['DownloadCategory']['parent_id']) {
               ...
               // trace($child->path );
                ...
                if ($child->appendTo($root)) {
                    mkdir('images/' . $child->path, '766');
                    $this->redirect(array('download/create', 'id' => $child->id));
                }
            } else {
                $model->attributes = $_POST['DownloadCategory'];
                $model->path = 'download/' . Yii::app()->landa->urlParsing($model->name) . '/';
                mkdir('images/' . $model->path, '766');
                if ($model->saveNode())
                    $this->redirect(array('download/create', 'id' => $model->id));
            }
        }
  ...
  public function actionUpdate($id) {
   ...
   public function actionDelete($id) {
    ...
    public function actionGenerateExcel() {
        ...
        if (isset($session['DownloadCategory_records'])) {
            $model = $session['DownloadCategory_records'];
        }
        else
            $model = DownloadCategory::model()->findAll();
    ...
    public function actionGeneratePdf() {
        ...
        $pdf->SetTitle('Laporan DownloadCategory');
        $pdf->SetSubject('Laporan DownloadCategory Report');
        //$pdf->SetKeywords('example, text, report');
        $pdf->SetHeaderData('', 0, "Report", '');
        //$pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, "Laporan" DownloadCategory, "");
        $pdf->SetHeaderData("", "", "Laporan DownloadCategory", "");
        ...
        $pdf->LastPage();
        $pdf->Output("DownloadCategory_002.pdf", "I");
    }
