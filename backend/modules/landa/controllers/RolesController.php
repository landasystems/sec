    ...
    public function accessRules() {
        return array(
            array('allow', // c
                ...
                'expression' => 'app()->controller->isValidAccess("GroupEmployment","c")',
                'expression' => 'app()->controller->isValidAccess("GroupContact","c")',
                'expression' => 'app()->controller->isValidAccess("GroupCustomer","c")'
            ),
            array('allow', // r
                'actions' => array('index', 'view'),
                'expression' => 'app()->controller->isValidAccess("GroupUser","r") || app()->controller->isValidAccess("GroupContact","r")',
...
//                'expression' => 'app()->controller->isValidAccess("GroupEmployment","r")',
//                'expression' => 'app()->controller->isValidAccess("GroupContact","r") || app()->controller->isValidAccess("GroupUser","r")',
//                'expression' => 'app()->controller->isValidAccess("GroupCustomer","r")'
            ...
            array('allow', // u
                ...
                'expression' => 'app()->controller->isValidAccess("GroupEmployment","u")',
            ...
            array('allow', // d
                ...
                'expression' => 'app()->controller->isValidAccess("GroupEmployment","d")',
    ...
    public function actionDelete($id, $type = 'user') {
        ...
        if (Yii::app()->request->isPostRequest) {
            ...
            if ($type == 'customer') {
                ...
                $client = json_decode($siteConfig->roles_client, true);
                $client = array_diff($client, array($id));
                $siteConfig->roles_client = json_encode($client);
            }
            ...
            //delete user
            User::model()->deleteAll(array('condition' => 'roles_id=' . $id));
...
//            $customer = $_GET['Roles']['$rolescust'];
    ...
    public function actionRolesAuth() {
        ...
        //insert from AUTH
        $arrMenu = Auth::model()->modules('Hotel Management Systems');
        $hms = $this->retAuth($arrMenu);
        $arrMenu = Auth::model()->modules('Content Management Systems');
        $cms = $this->retAuth($arrMenu);
        $arrMenu = Auth::model()->modules('Inventory Management Systems');
        $ims = $this->retAuth($arrMenu);

        $results = array_merge($hms, $cms, $ims);
        trace($results);
        foreach ($results as $key => $value) {
//            if(isset(strpos($key,'Report'))){
//                $a = array('r'=>1);
//            }else{
//                $a = array('c'=>1,'r'=>1,'u'=>1,'d'=>1);
//            }
//            echo strpos($key,'Report');
             $a = array('c'=>1,'r'=>1,'u'=>1,'d'=>1);
            $c= json_encode($a);
            $contact = new Auth;
            $contact->id = $key;
    ...
    public function retAuth($arrMenu) {
        $results = array();
            ...
            if (isset($arr['items'])) {
                foreach ($arr['items'] as $arrItems) {
                    if (isset($arrItems['auth_id'])) {
                        $results[$arrItems['auth_id']] = '';
                    }

                    if (isset($arrItems['items'])) {
                        foreach ($arrItems['items'] as $arrItems2) {
                            if (isset($arrItems2['auth_id'])) {
                                $results[$arrItems2['auth_id']] = '';
                            }
                        }
                    }
                }
